...
class ECBExchangeRateSensor(Entity):
    ...
    async def async_added_to_hass(self):
        """Start the periodic update when added to HA."""
    ...
    async def async_will_remove_from_hass(self):
        """Stop the periodic update when removed from HA."""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(ECB_URL) as response:
                    ...
                    if base in rates and quote in rates:
                        ...
                        _LOGGER.info("Setting state: %s (rounded: %s)", self._rate, round(self._rate, self._precision))
